...
{{"pass": true/false, "score": 0.0到1.0之间的浮点数, "reasoning": "简短的判断理由（中文，不超过50字）"}}"""
    ...
    MEMORY_JUDGE_PROMPT_TEMPLATE = """你是一个严格公正的 AI 评测裁判。你的任务是判断 Agent 当前记忆库内容是否已经语义上包含目标信息。
...
【判断规则】
- 语义等价或同义表达应视为通过（不要求字面完全一致）
- 若记忆中明确缺失目标信息，应判定为失败
- 若记忆内容与目标信息不相关，应判定为失败
...
{{"pass": true/false, "score": 0.0到1.0之间的浮点数, "reasoning": "简短的判断理由（中文，不超过50字）"}}"""
    ...
    def __init__(
        ...
        Args:
            ...
            temperature: 采样温度；None 时默认 0（确定性输出）。
                         部分模型（如 Kimi）只接受 temperature=1，需显式指定。
        """
        try:
    ...
    def judge_memory(
        ...
        memory_snapshot: Any,
        expected_keywords: List[str],
        check_name: str = "llm_memory_judge_check",
    ) -> SubCheckResult:
        """
        让 LLM 裁判对记忆库快照进行语义评估。

        Args:
            memory_snapshot:   GET /memory 返回的 memories 内容（任意结构）
            expected_keywords: 期望记忆中已包含的关键信息
            check_name:        子检查名称（显示在报告中）
        ...
        if isinstance(memory_snapshot, str):
            memory_str = memory_snapshot
        else:
            memory_str = json.dumps(memory_snapshot, ensure_ascii=False)

        prompt = self.MEMORY_JUDGE_PROMPT_TEMPLATE.format(
            expected_keywords=", ".join(expected_keywords),
            memory_snapshot=memory_str,
        )
